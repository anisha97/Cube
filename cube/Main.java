...
public class Main implements Observer {
    ...
    private void initPuzzle() {
        for (int i = 0; i < 6; i++) {
            int randomNum = ThreadLocalRandom.current().nextInt(0,  _model._side * _model._side);

            int row = randomNum / _model._side;
            int col = randomNum % _model._side;

            if (_model.boardPainted[row][col]) {
                i--;
            } else {
                _model.boardPainted[row][col] = true;
